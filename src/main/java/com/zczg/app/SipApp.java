...
import com.zczg.util.MediaConf;
					...
					if (event.isSuccessful()) {
						if (fromUser.compareState(toName, SipUser.WAITING_FOR_MEDIA_SERVER)) {
							if (linkedSession != null) {
								if (toUser.compareState(fromName, SipUser.INIT_BRIDGE)) {
									...
									if (inviteForCallee != null) {
										...
										// inviteForCallee.setContent(sdp,
										// "application/sdp");
							...
							if (ConfData.isConf(toName)) {
								...
								// A想和B恢复通话，A恢复的时候，如果B是holdon-guest，那就可以直接恢复，申请conn前B的状态改为了calling，所以A看到B的状态为calling，自己的状态也是calling才对
								if (!ConfData.isConf(toName)) {
